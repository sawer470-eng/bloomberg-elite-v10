...
def render_options_data():
    ...
    with st.spinner("Fetching options chain..."):
        try:
            exp_date, chain = get_options_data(ticker)
            
            if chain is None or (hasattr(chain, 'puts') and chain.puts.empty):
                st.markdown("""
                <div class="metric-card" style="border-left: 4px solid #FF9900;">
                    <h4 style="color:#FF9900; margin:0;">[DATA UNAVAILABLE]</h4>
                    <p style="color:#8b949e; margin:5px 0 0 0;">Yahoo Finance Options API is currently not returning data for this symbol in the Cloud environment. 
                    Positioning data will restore automatically upon API refresh.</p>
                </div>
